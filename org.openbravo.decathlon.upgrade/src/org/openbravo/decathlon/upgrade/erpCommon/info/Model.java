...
public class Model extends HttpSecureAppServlet {
  private static final long serialVersionUID = 1L;

  private static final String[] colNames = { "value", "name", "modelcode", "rowkey" };
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException,
    ...
    if (vars.commandIn("DEFAULT")) {
      ...
      String strKey = "";
      String strWindow = vars.getGlobalVariable("WindowID", "Model.windowId", "");
      /*String strBpartner = vars.getGlobalVariable("inpBpartnerId", "Model.bpartner", ""); */
      String strNameValue = vars.getGlobalVariable("inpNameValue", "Model.key", "");
      vars.setSessionValue("Model.adorgid", vars.getStringParameter("inpAD_Org_ID", ""));
      vars.removeSessionValue("Model.key");
      if (!strNameValue.equals("")) {
        ...
        if (guion != -1) {
          ...
          vars.setSessionValue("Model.key", strKey);
        }
        vars.setSessionValue("Model.modelname", strNameValue + "%");
      }
      ...
      String strWindow = vars.getGlobalVariable("WindowID", "Model.windowId", "");
     /* String strBpartner = vars.getGlobalVariable("inpBpartnerId", "Model.bpartner", ""); */
      String strKeyValue = vars.getGlobalVariable("inpNameValue", "Model.key", "");
      // getGlobalVariable only used to store request value into session, not to read it from there
      String strOrg = vars.getGlobalVariable("inpAD_Org_ID", "Model.adorgid", "");
      vars.setSessionValue("Model.key", strKeyValue + "%");
      ModelData[] data = ModelData.selectKey(this, Utility.getContext(this, vars,
          "#User_Client", "Model"), Utility.getSelectorOrgs(this, vars, strOrg), strKeyValue + "%");
      ...
      String strWindowId = vars.getGlobalVariable("inpWindowId", "Model.windowId", "");
      String strKey = vars.getGlobalVariable("inpKey", "Model.key", "");
      String strName = vars.getGlobalVariable("inpName", "Model.modelname", "");
      String strModelID = vars.getGlobalVariable("inpModelID", "Model.modelcode", "");
      /*String strBpartners = vars.getGlobalVariable("inpBpartnerId", "Model.bpartner", "");*/
      ...
      printGridData(response, vars, strKey, strName, strModelID, strSortCols, strSortDirs,
  ...
  private void removePageSessionVariables(VariablesSecureApp vars) {
    vars.removeSessionValue("Model.key");
    vars.removeSessionValue("Model.modelname");
    vars.removeSessionValue("Model.modelcode");
   /* vars.removeSessionValue("Model.bpartner"); */
    vars.removeSessionValue("Model.currentPage");
    // remove saved adorgid only when called from DEFAULT,KEY
    // but not when called by clicking search in the selector
    if (!vars.getStringParameter("newFilter").equals("1")) {
      vars.removeSessionValue("Model.adorgid");
    }
  }
  ...
  private void printPage(HttpServletResponse response, VariablesSecureApp vars, String strKeyValue,
    ...
    if (log4j.isDebugEnabled())
      log4j.debug("Output: Frame 1 of the Models seeker");
    XmlDocument xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/decathlon/upgrade/erpCommon/info/Model")
        .createXmlDocument();
    ...
    xmlDocument.setParameter("tercero", ModelData.selectTercero(this, strBpartners));*/
  ...
  private void printPageKey(HttpServletResponse response, VariablesSecureApp vars,
      ModelData[] data) throws IOException, ServletException {
    if (log4j.isDebugEnabled())
      log4j.debug("Output: Model seeker Frame Set");
  ...
  private String generateResult(ModelData[] data) throws IOException, ServletException {
    StringBuffer html = new StringBuffer();

    html.append("\nfunction validateSelector() {\n");
    html.append("var key = \"" + data[0].clModelId + "\";\n");
    html.append("var text = \""
       + Replace.replace((data[0].value + " - " + data[0].name + " - " + data[0].modelcode), "\"", "\\\"") + "\";\n");
  ...
  private void printGridData(HttpServletResponse response, VariablesSecureApp vars, String strKey,
      String strName, String strModelID, String strOrderCols, String strOrderDirs,
    ...
    if (headers != null) {
      try {
        ...
        page = TableSQLData.calcAndGetBackendPage(vars, "Model.currentPage");
        ...
        strNumRows = ModelData.countRows(this, rownum, Utility.getContext(this, vars, "#User_Client", "Model"), Utility.getSelectorOrgs(this, vars, strOrg), strKey, strName, strModelID, pgLimit, oraLimit1, oraLimit2); 
          //strNumRows = String.valueOf(data.length);
          vars.setSessionValue("ModelData.numrows", strNumRows);
        } else {
          strNumRows = vars.getSessionValue("ModelData.numrows");
        }

        // Filtering result
        if (this.myPool.getRDBMS().equalsIgnoreCase("ORACLE")) {
          String oraLimit = (offset + 1) + " AND " + String.valueOf(offset + pageSize);
          data = ModelData.select(this, "ROWNUM", Utility.getContext(this,
              vars, "#User_Client", "Model"), Utility.getSelectorOrgs(this, vars, strOrg),
              strKey, strName, strModelID, strOrderBy, oraLimit, "");
        } else {
          String pgLimit = pageSize + " OFFSET " + offset;
          data = ModelData.select(this, "1", Utility.getContext(this, vars,
              "#User_Client", "Model"), Utility.getSelectorOrgs(this, vars, strOrg), strKey,
              strName, strModelID, strOrderBy, "", pgLimit);
        }
  ...
  public String getServletInfo() {
    return "Servlet that presents the Model seeker";
