 ...
 ************************************************************************
 */
package org.openbravo.decathlon.upgrade.erpCommon.info;
...
public class Company extends HttpSecureAppServlet {
  private static final long serialVersionUID = 1L;

  private static final String[] colNames = { "value", "name", "companyid", "rowkey" };
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException,
    ...
    if (vars.commandIn("DEFAULT")) {
      ...
      String strKey = "";
      String strWindow = vars.getGlobalVariable("WindowID", "Company.windowId", "");
      /*String strBpartner = vars.getGlobalVariable("inpBpartnerId", "Company.bpartner", ""); */
      String strNameValue = vars.getGlobalVariable("inpNameValue", "Company.key", "");
      vars.setSessionValue("Company.adorgid", vars.getStringParameter("inpAD_Org_ID", ""));
      vars.removeSessionValue("Company.key");
      if (!strNameValue.equals("")) {
        ...
        if (guion != -1) {
          ...
          vars.setSessionValue("Company.key", strKey);
        }
        vars.setSessionValue("Company.companyname", strNameValue + "%");
      }
      printPage(response, vars, strKey, strNameValue + "%", strWindow, "paramName");
    } else if (vars.commandIn("KEY")) {
      removePageSessionVariables(vars);
      String strWindow = vars.getGlobalVariable("WindowID", "Company.windowId", "");
     /* String strBpartner = vars.getGlobalVariable("inpBpartnerId", "Company.bpartner", ""); */
      String strKeyValue = vars.getGlobalVariable("inpNameValue", "Company.key", "");
      // getGlobalVariable only used to store request value into session, not to read it from there
      String strOrg = vars.getGlobalVariable("inpAD_Org_ID", "Company.adorgid", "");
      vars.setSessionValue("Company.key", strKeyValue + "%");
      CompanyData[] data = CompanyData.selectKey(this, Utility.getContext(this, vars,
          "#User_Client", "Company"), Utility.getSelectorOrgs(this, vars, strOrg), strKeyValue + "%");
      if (data != null && data.length == 1) {
        printPageKey(response, vars, data);
      } else
        printPage(response, vars, strKeyValue + "%", "", strWindow, "paramKey");
      ...
      String strWindowId = vars.getGlobalVariable("inpWindowId", "Company.windowId", "");
      String strKey = vars.getGlobalVariable("inpKey", "Company.key", "");
      String strName = vars.getGlobalVariable("inpName", "Company.companyname", "");
      String strCompanyID = vars.getGlobalVariable("inpCompanyID", "Company.companyid", "");
      /*String strBpartners = vars.getGlobalVariable("inpBpartnerId", "Company.bpartner", "");*/
      ...
      printGridData(response, vars, strKey, strName, strCompanyID, strSortCols, strSortDirs,
  ...
  private void removePageSessionVariables(VariablesSecureApp vars) {
    vars.removeSessionValue("Company.key");
    vars.removeSessionValue("Company.companyname");
    vars.removeSessionValue("Company.companyid");
   /* vars.removeSessionValue("Company.bpartner"); */
    vars.removeSessionValue("Company.currentPage");
    // remove saved adorgid only when called from DEFAULT,KEY
    // but not when called by clicking search in the selector
    if (!vars.getStringParameter("newFilter").equals("1")) {
      vars.removeSessionValue("Company.adorgid");
    }
  }

  private void printPage(HttpServletResponse response, VariablesSecureApp vars, String strKeyValue,
      String strNameValue, String strWindow, String focusedId)
      throws IOException, ServletException {
    if (log4j.isDebugEnabled())
      log4j.debug("Output: Frame 1 of the Companys seeker");
    XmlDocument xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/decathlon/upgrade/erpCommon/info/Company")
        .createXmlDocument();
    ...
    xmlDocument.setParameter("tercero", CompanyData.selectTercero(this, strBpartners));*/
  ...
  private void printPageKey(HttpServletResponse response, VariablesSecureApp vars,
      CompanyData[] data) throws IOException, ServletException {
    if (log4j.isDebugEnabled())
      log4j.debug("Output: Company seeker Frame Set");
  ...
  private String generateResult(CompanyData[] data) throws IOException, ServletException {
    StringBuffer html = new StringBuffer();

    html.append("\nfunction validateSelector() {\n");
    html.append("var key = \"" + data[0].rcCompanyId + "\";\n");
    html.append("var text = \""
       + Replace.replace((data[0].value + " - " + data[0].name + " - " + data[0].companyid), "\"", "\\\"") + "\";\n");
  ...
  private void printGridData(HttpServletResponse response, VariablesSecureApp vars, String strKey,
      String strName, String strCompanyID, String strOrderCols, String strOrderDirs,
    ...
    if (headers != null) {
      try {
        ...
        page = TableSQLData.calcAndGetBackendPage(vars, "Company.currentPage");
        ...
        strNumRows = CompanyData.countRows(this, rownum, Utility.getContext(this, vars, "#User_Client", "Company"), Utility.getSelectorOrgs(this, vars, strOrg), strKey, strName, strCompanyID, pgLimit, oraLimit1, oraLimit2); 
          //strNumRows = String.valueOf(data.length);
          vars.setSessionValue("CompanyData.numrows", strNumRows);
        } else {
          strNumRows = vars.getSessionValue("CompanyData.numrows");
        }

        // Filtering result
        if (this.myPool.getRDBMS().equalsIgnoreCase("ORACLE")) {
          String oraLimit = (offset + 1) + " AND " + String.valueOf(offset + pageSize);
          data = CompanyData.select(this, "ROWNUM", Utility.getContext(this,
              vars, "#User_Client", "Company"), Utility.getSelectorOrgs(this, vars, strOrg),
              strKey, strName, strCompanyID, strOrderBy, oraLimit, "");
        } else {
          String pgLimit = pageSize + " OFFSET " + offset;
          data = CompanyData.select(this, "1", Utility.getContext(this, vars,
              "#User_Client", "Company"), Utility.getSelectorOrgs(this, vars, strOrg), strKey,
              strName, strCompanyID, strOrderBy, "", pgLimit);
        }
  ...
  public String getServletInfo() {
    return "Servlet that presents the Company seeker";
