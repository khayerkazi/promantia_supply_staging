...
public class Transfer extends DalBaseProcess {

  final private static Logger log = Logger.getLogger(Transfer.class);
  ...
  protected void doExecute(ProcessBundle bundle) throws Exception {
    ...
    try {
      ...
      processValidation(shipmentInOut);
  ...
  public void processValidation(ShipmentInOut shipmentInOut) throws Exception, JSONException {
    ...
    Order returnToVendorHeader = createRTVHeader(shipmentInOut);
    ShipmentInOut returnMaterialShipmentHeader = createRMShipmentHeader(shipmentInOut, rtvsDocType);
    ...
    for (ShipmentInOutLine ioLine : listShipmentInOutLine) {
      ...
      try {
        ...
        for (OrderLine ordLine : ordLineList) {
          ...
          // If the record is implantation set blocked qty to received qty
          /*
           * if (ordLine.getSalesOrder().isSwIsimplantation()) { // Setting blocked qty if PO is of
           * type implantation BigDecimal blockedQty = new BigDecimal(strActualMovementQty);
           * CLImplantation implRecord = BusinessEntityMapper.getImplantationOrg(
           * ordLine.getOrganization(), ordLine.getProduct());
  ...
  public void SaveInventoryHeader(InventoryCount inventoryHeader) {
    try {
      OBDal.getInstance().save(inventoryHeader);
    } catch (Exception e) {
      log.error(e);
      e.printStackTrace();
    }
  ...
  public List<InventoryCountLine> createInventory(InventoryCount inventoryHeader,
      ShipmentInOutLine ioLine, ProcessBundle bundle, double qty) {

    List<InventoryCountLine> inventoryInOutLine = new ArrayList<InventoryCountLine>();
    long line = 10;
    try {
      BigDecimal qtyToBeOrdered = BigDecimal.valueOf(qty);
      Product pr = ioLine.getProduct();

      InventoryCountLine newInventoryCountLine = OBProvider.getInstance().get(
          InventoryCountLine.class);
      newInventoryCountLine.setLineNo(line);
      newInventoryCountLine.setClient(inventoryHeader.getClient());
      newInventoryCountLine.setOrganization(inventoryHeader.getOrganization());
      newInventoryCountLine.setActive(true);
      newInventoryCountLine.setCreationDate(new Date());
      newInventoryCountLine.setCreatedBy(getUser(SOConstants.User));
      newInventoryCountLine.setUpdatedBy(getUser(SOConstants.User));
      newInventoryCountLine.setUpdated(new Date());
      newInventoryCountLine.setBookQuantity(qtyToBeOrdered);
      newInventoryCountLine.setOrderQuantity(qtyToBeOrdered);
      newInventoryCountLine.setPhysInventory(inventoryHeader);
      newInventoryCountLine.setProduct(pr);
      newInventoryCountLine.setUOM(pr.getUOM());

    } catch (Exception e) {
      log.error(e);
      e.printStackTrace();
    }
    return inventoryInOutLine;
  }

  public InventoryCount createInventoryHeader(ShipmentInOutLine ioLine, ProcessBundle bundle) {
    InventoryCount inventoryHeader = OBProvider.getInstance().get(InventoryCount.class);
    try {
      inventoryHeader.setClient(ioLine.getClient());
      inventoryHeader.setOrganization(ioLine.getOrganization());
      inventoryHeader.setActive(true);
      inventoryHeader.setCreationDate(new Date());
      inventoryHeader.setCreatedBy(ioLine.getCreatedBy());
      inventoryHeader.setUpdatedBy(ioLine.getUpdatedBy());
      inventoryHeader.setUpdated(new Date());
      inventoryHeader.setWarehouse(getWarehouse(disputeBin));
      inventoryHeader.setMovementDate(new Date());
  ...
  public String getNextAssignedNo(String docName, Organization org) {
    String qry = "id in (select id from DocumentType d where d.name='" + docName
        + "' and d.organization.id='" + org.getId() + "')";
    OBQuery<DocumentType> strQry = OBDal.getInstance().createQuery(DocumentType.class, qry);
    strQry.setMaxResult(1);
    List<DocumentType> DocumentTypelst = strQry.list();
    String docno = "";
    for (DocumentType dt : DocumentTypelst) {
      docno = dt.getDocumentSequence().getCurrentNextSystem().toString();
    }
    return docno;
  }

  private Order createRTVHeader(ShipmentInOut io) throws Exception {
    ...
    try {
      ...
      newOrder.setCreatedBy(io.getCreatedBy());
      newOrder.setUpdatedBy(io.getUpdatedBy());
  ...
  public List<OrderLine> createRTVOrderLine(Order newOrder, ShipmentInOutLine ioLine, double qty) {
    ...
    newOrderLine.setCreatedBy(getUser(SOConstants.User));
    newOrderLine.setUpdatedBy(getUser(SOConstants.User));
