...
public class CompleteActionHandler extends BaseActionHandler {
  ...
  protected JSONObject execute(Map<String, Object> parameters, String content) {
    ...
    log.debug("Inside CompleteActionHandler");
    try {
      JSONObject jsonRequest = new JSONObject(content);
      final String strAction = jsonRequest.getString("action");
      if (strAction.equals("complete")) {
        JSONObject jsonMsg = complete(jsonRequest);
  ...
  private JSONObject complete(JSONObject jsonRequest) throws JSONException, PropertyException {
    ...
    for (int i = 0; i < pickingIds.length(); i++) {
      ...
      if (!picking.getPickliststatus().equals("AS")) {
        throw new OBException(OBMessageUtils.messageBD("IBUDPK_CompleteProcessStatusError"));
      } else {
        ...
        if (picklistLines.size() >= 0) {
          executeProcessForZeroLines(picking);
          if (picklistLines.size() == 0) {
            picking.setPickliststatus(SOConstants.closed);
            jsonMsg
                .put(
                    "text",
                    OBMessageUtils
                        .messageBD("Since there are no lines, status of picklist and SO changes to closed "));
          }
        }

        for (InternalMovementLine line : picklistLines) {
          MovementLineHandler.processMovementLine(line.getId(), "CF", line.getMovementQuantity(),
              "confirm");
        }
      }
