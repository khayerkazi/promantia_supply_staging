...
public class GetDscWHLocationIDList extends HttpSecureAppServlet
		implements WebService {
	
	private static final long serialVersionUID = 1L;
	
	static Logger log4j = Logger.getLogger(GetDscWHLocationIDList.class);
	...
	public static JSONObject parseXML(Document xml) {
		...
		try {
			...
			boxNumbers = xml.getElementsByTagName("Location").item(0)
					.getChildNodes().item(0).getNodeValue();
			
			List<String> list = new ArrayList<String>(Arrays.asList(boxNumbers.split(",")));

			final Query queryForDscBoxId = session
					.createSQLQuery("select distinct location_barcode from dsc_wh_location where location_no in (:locationInList)");
			
			queryForDscBoxId.setParameterList("locationInList", list);
