...
/*global OB, console, enyo, exports */
...
(function () {
  ...
  function EnyoNode(enyoGeneratedIdDOM, preGeneratedIdTest) {
    ...
    var initIdTest = function () {
      idTest = {
        ...
        isUnique: false, // true if unique, false if not
    ...
    this.setFirstInvertedUniqueSegment = function (invertedIndex) {
      if (isInNeedOfNewId) {
        ...
        if (newFirstUniqueSegment + minimumSegments < 0) {
          // this could happen if there are repeated ids in the DOM
          // must be handle by the caller
          throw {
            name: "Fatal Error",
            message: "Your are trying to set the first valid segment to a non existant segment"
          };
        }
  ...
  function scan() {
        ...
        if (enyoNodesToProcess.hasOwnProperty(nodeToProcessIndex)) {
          ...
          if (!isFound) {
            var segmentToProcess2 = {
              tail: tail,
              count: 1,
              enyoNodeToProcessNodeIndex: nodeIndex, // this is only used when its unique
    ...
    if (untesteableIdTests.length > 0) {
      OB.info("found %s ids of %s that are not valid for unit tests. execute printUntesteableIds() to get the list\n", untesteableIdTests.length, validEnyoObjects);
  ...
  function getList() {
    ...
    if (enyoNodes === null) {
      ...
      scan();
      OB.info("totalObjects: %s, enyoObjects: %s, nullCount: %s, enyo ids to change: %s, validObjects: %s, id modifications: %s, maxSegments: %s\n", totalEnyoObjects, enyoObjectsCount, nullCount, needsTheIdToBeRemovedInEnyoComponentCreation, validEnyoObjects, idModifications, maxSegments);
  ...
  TestRegistry.printUntesteableIds = function () {
    ...
    for (index = 0; index < untesteableIdTests.length; index++) {
      ...
      OB.info("'%s' in '%s'", notValidForUnitTestRow.incompatibleStringDetected, notValidForUnitTestRow.id);
    }
    OB.info("Total untesteable ids: %s\n", untesteableIdTests.length);
  };
   ...
   */
  TestRegistry.registry = function (idTest) {
    var enyoNode = null;
    var rescan = true;
    if (enyoNodes === null) {
      rescan = false;
    ...
    OB.error("Cannot find any object with idTest = '" + idTest + "'");
    return null;
  };
  ...
  TestRegistry.appendIdTestToDOM = function () {
    ...
    if (attributeAppendErrors > 0) {
      OB.info("DOM append errors: %s\n", attributeAppendErrors);
    }
  };
