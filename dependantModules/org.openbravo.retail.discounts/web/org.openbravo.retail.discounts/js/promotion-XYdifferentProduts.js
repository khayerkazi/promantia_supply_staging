  ...
  implementation: function (discountRule, receipt, line) {
    ...
    if (OB.DEC.toNumber(qty) >= x) {
      ...
      for (i = 0; i < group.length; i++) {
        ...
        if (subtype === 'CHEAPEST') {
          if (OB.DEC.toNumber(checkedQty) < nextY) {
            ...
            discountRule.set('qtyOffer', OB.DEC.toNumber(checkedQty));
            ...
            if (distributed) {
              ...
              discountRule.set('qtyOffer', discountedQty);
              receipt.addPromotion(ln, discountRule, {
      ...
      if (distributed && (subtype === 'CHEAPEST')) {
        unitsToDiscount = chunks * (x - y);
        var totalQty = chunks * x;
        var pendingQty = totalQty;
        var qtyOffer = 0;
        var partialAmt = 0;
        for (i = 0; i < applyingToLines.length; i++) {
          ln = applyingToLines.at(i);
          price = OB.DEC.toBigDecimal(ln.get('discountedLinePrice') || ln.get('price'));
          amt = OB.DEC.toNumber(OB.DEC.toBigDecimal(ln.get('qtyToApplyDisc')).multiply(price).multiply(distributionAmt).multiply(OB.DEC.toBigDecimal(unitsToDiscount)).divide(totalPrice.multiply(OB.DEC.toBigDecimal(distributionQty))));
          if ((i === applyingToLines.length - 1) && ((partialAmt + amt) !== distributionAmt)) {
            amt = OB.DEC.toNumber(distributionAmt.subtract(OB.DEC.toBigDecimal(partialAmt)));
          } else {
            partialAmt = OB.DEC.add(partialAmt, amt);
          }

          // calculate the qty of line is used in this discount
          if (pendingQty <= ln.get('qtyToApplyDisc')) {
            qtyOffer = pendingQty;
            pendingQty = 0;
          } else {
            qtyOffer = ln.get('qtyToApplyDisc');
            pendingQty -= ln.get('qtyToApplyDisc');
          }
          discountRule.set('qtyOffer', qtyOffer);
          receipt.addPromotion(ln, discountRule, {
            actualAmt: amt,
            amt: distribution[i]
          });
