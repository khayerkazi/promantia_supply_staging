...
public class CostingJob {
	...
	private final String jobName = "CostingJob";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass()
						...
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(CostingJob.this, new Object[] { e,
									globalMap });
							break;
						}
					}
			...
			synchronized (commonByteArrayLock) {
				...
				try {
					...
					if (length == -1) {
						...
						dis.readFully(commonByteArray, 0, length);
						this.StartingDate = new String(commonByteArray, 0,
								length);
					}
					...
					if (length == -1) {
						...
						dis.readFully(commonByteArray, 0, length);
						this.TotalMovementQuantity = new String(
		...
		try {
			...
			while (limittFileInputDelimited_2 != 0
				...
				if (row1 != null) {
					...
					row2.CostType = validator.checkNotNull(validator
							.checkString(row1.CostType, 32, "CostType"),
							"CostType");
					row2.Cost = validator.checkNotNull(validator
							.checkBigDecimal(row1.Cost), "Cost");
					row2.StartingDate = validator.checkNotNull(validator
							.checkDate(row1.StartingDate, "StartingDate"),
							"StartingDate");
					row2.EndingDate = validator.checkNotNull(validator
							.checkDate(row1.EndingDate, "EndingDate"),
							"EndingDate");
					row2.Quantity = validator.checkBigDecimal(row1.Quantity);
					row2.TotalMovementQuantity = validator
							.checkBigDecimal(row1.TotalMovementQuantity);
					...
					if (row4 != null) {
						...
						row7.whyError += service
								.Utility_messageBD("IDL_BAD_REFERENCE")
								+ row4.Product;
						Warehouse warehouse = service.findDALInstance(true,
								Warehouse.class, new Value(
										Warehouse.PROPERTY_SEARCHKEY,
										row4.Warehouse));
						row7.flag = warehouse == null ? "-1" : "0";
						row7.whyError += service
								.Utility_messageBD("IDL_BAD_REFERENCE")
								+ row4.Warehouse;
						Currency currency = service.findDALInstance(true,
								Currency.class, new Value(
										Currency.PROPERTY_ISOCODE,
										row4.Currency));
						row7.flag = (currency == null && row4.Currency != null) ? "-1"
								: "0";
						row7.whyError += service
								.Utility_messageBD("IDL_BAD_REFERENCE")
								+ row4.Currency;
						...
						if (row3 != null) {
							...
							service3.finishRecordProcess(row3.Organization,
									row3.Product, row3.CostType, row3.Cost,
									row3.StartingDate, row3.EndingDate,
									row3.Quantity, row3.TotalMovementQuantity,
									row3.Price, row3.Warehouse, row3.Currency);
						...
						if (row8 != null) {
							...
							service6.logRecordError(row8.whyError,
									row8.Organization, row8.Product,
									row8.CostType, row8.Cost,
									row8.StartingDate, row8.EndingDate,
									row8.Quantity, row8.TotalMovementQuantity,
									row8.Price, row8.Warehouse, row8.Currency);
					...
					if (row5 != null) {
						...
						service2.logRecordError(row5.whyError,
								row5.Organization, row5.Product, row5.CostType,
								row5.Cost, row5.StartingDate, row5.EndingDate,
								row5.Quantity, row5.TotalMovementQuantity,
								row5.Price, row5.Warehouse, row5.Currency);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			java.io.InputStream inContext = CostingJob.class
					.getClassLoader()
					.getResourceAsStream(
			...
			if (contextStr.compareTo("Default") != 0) {
				inContext = CostingJob.class.getClassLoader()
						.getResourceAsStream(
								"initial_data_load/costingjob_0_1/contexts/"
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory)
					+ " bytes memory increase when running : CostingJob");
		}
...
/************************************************************************************************
 * 139747 characters generated by Talend Open Studio on the April 11, 2013
 * 1:51:29 PM CEST
 ************************************************************************************************/
