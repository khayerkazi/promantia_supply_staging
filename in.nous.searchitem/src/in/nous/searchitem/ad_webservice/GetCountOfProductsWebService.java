...
public class GetCountOfProductsWebService implements WebService {
  ...
  public void doGet(String path, HttpServletRequest request, HttpServletResponse response)
    ...
    String code = URLDecoder.decode(request.getParameter("code").toString(), "UTF-8");
    ...
    { 
	 ...
	 {
		 hql = " SELECT count(*) as count from MaterialMgmtMaterialTransaction where product.name = '"+code+"' and 	organization.name='"+storename+"' and  swMovementtype in ('DO', 'CE','TRI','POS','TRO','IC','I','PI','WT','D','IRC','WD','SRQ','SRN')" +
         		" and updated > '"+date+"' and storageBin.searchKey like '%Saleable%'";
		 
	 }else if(selected.equalsIgnoreCase("I")){
		 
		 hql = "SELECT count(*) as count from MaterialMgmtMaterialTransaction where product.name = '"+code+"' and 	organization.name='"+storename+"' and " +
     	 		"(swMovementtype='"+selected+"' or swMovementtype='PI') and " +
     	 		"updated > '"+date+"' and storageBin.searchKey like '%Saleable%'";
		 
	 }else if(selected.equalsIgnoreCase("WD")){
		 
		 hql = "SELECT count(*) as count from MaterialMgmtMaterialTransaction where product.name = '"+code+"' and 	organization.name='"+storename+"' and " +
	     	 		"(swMovementtype='"+selected+"' or swMovementtype='SRN') and " +
	     	 		"updated > '"+date+"' and storageBin.searchKey like '%Saleable%'";
		 
	 }else{
		 
		 hql = " SELECT count(*) as count from MaterialMgmtMaterialTransaction where product.name = '"+code+"' and 	organization.name='"+storename+"' and swMovementtype='"+selected+"' and updated > '"+date+"' and storageBin.searchKey like '%Saleable%'";
